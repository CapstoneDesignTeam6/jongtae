...
- question     : 문제 (토론에서 실제 다룬 사실·수치·사건 기반)
- explanation  : 정답 전체 해설 (2~3문장)
...
class WeaknessQuizAgent:
    ...
    def _analyze_weakness(self, history_block: str, topic: str) -> str:
        """history 전체 분석 → 유저가 헷갈리거나 논리가 약했던 부분 추출."""
        system = f"""당신은 시사 토론 분석가입니다.
유저({self.user_label})의 토론 발언 전체를 분석하여
가장 논리가 약하거나 헷갈려한 부분 1가지를 찾아내세요.

출력 규칙:
- 2~3문장으로 구체적으로 서술
- 어떤 개념·사실·수치를 잘못 이해하거나 근거 없이 주장했는지 명시
- 번호·불릿 없이 바로 본문만 출력"""
    ...
    def _generate_quiz(
        ...
        """약점 기반 퀴즈 생성."""
        system = """당신은 시사 토론 퀴즈 출제자입니다.
유저가 헷갈려하거나 약했던 부분을 정확히 이해했는지 확인하는 4지선다 퀴즈 1개를 만드세요.
...
- distractors  : 오답 텍스트 배열 (정확히 3개)
- wrong_explanations: 오답 3개에 대한 설명 배열 (distractors와 순서 동일, 정확히 3개)
- question     : 문제 (유저 약점과 직접 연관된 사실·개념 기반)
- explanation  : 정답 전체 해설 (2~3문장)
- correct_explanation: 이 보기가 왜 정답인지 (1문장)
