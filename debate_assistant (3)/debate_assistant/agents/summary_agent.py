...
class SummaryAgent:
    ...
    def _filter_invalid_turns(self, history, topic):
        user_turns = []
        user_turn_num = 0
        for h in history:
            if h["role"] == "user":
...
- JSON 배열 외 아무것도 출력하지 말 것"""
    ...
    def _summarize_core(self, history_block, topic, json_block, turns, invalid_contents):
        invalid_note = ""
        if invalid_contents:
            previews = []
...
(AI 발언에서 주장·근거 문장을 그대로 복사. 줄 나눔.)"""
        ...
        extracted = call_ollama(extract_system, f"토론 기록:\n{history_block}")
        print(f"  └─ 추출 결과:\n{extracted[:300]}")

        # 2단계: 핵심 논점 구조화
        structure_system = f"""당신은 토론 분석가입니다.
[추출된 발언]을 읽고, 양측이 실제로 충돌한 핵심 논점 2~3개를 파악하세요.

아래 형식으로만 출력하세요. 새로운 내용·수치 추가 절대 금지.
...
[{self.user_label} 핵심 주장]
(가장 강조한 주장 2~3개, 추출된 발언 기반으로만)

[{self.ai_label} 핵심 주장]
(가장 강조한 주장 2~3개, 추출된 발언 기반으로만)"""

        structured = call_ollama(structure_system, f"추출된 발언:\n{extracted}")
        print(f"  └─ 구조화 결과:\n{structured[:300]}")

        # 3단계: 서술형 요약 생성
        polish_system = f"""당신은 시사 토론 해설자입니다.
[추출된 발언]과 [논점 구조]를 바탕으로 토론 요약문을 작성하세요.

작성 규칙:
- [추출된 발언]에 있는 내용만 사용. 새로운 사실·수치·단어 추가 절대 금지.
- 추측·단정 표현("~할 것입니다", "~될 것입니다") 절대 금지.
- 대신 "~라고 주장했습니다", "~라고 강조했습니다", "~를 근거로 들었습니다",
  "~라는 입장을 내세웠습니다" 처럼 인용 형식으로 서술.
- 단순 나열 금지. 주장들이 논리적 흐름으로 이어지도록 서술.
- 상대 주장을 받아칠 때는 레이블·콜론·줄바꿈 없이 문단 흐름 안에서 자연스럽게 연결.
- "재반박", "반박" 같은 메타 표현 절대 금지.
- 각 측 문단은 200~350자.
...
(서술형 문단)"""
    ...
    def _generate_feedback_with_search(
        self, history_block, topic, json_block, turns, summary_context
    ):
...
쉽고 자연스러운 말로. 지시형 금지. 관찰·분석만."""
        ...
        supplement = self._build_supplement(search_results, logic_feedback, topic, history_block)
        final_feedback = f"{logic_feedback}\n\n{supplement}" if supplement else logic_feedback
        return final_feedback, raw_feedback

    def _generate_queries_with_ollama(self, logic_feedback, topic):
...
- Example format: ["query1", "query2", "query3", "query4", "query5"]"""
        ...
        try:
            ...
            if match:
                ...
                return [q for q in queries if isinstance(q, str) and all(ord(c) < 128 for c in q)][:5]
        except Exception as e:
            print(f"  └─ 쿼리 파싱 실패: {e}")
        return []

    def _run_tavily_search(self, queries):
        all_results = []
        for q in queries:
            ...
            try:
                ...
                if isinstance(raw, str) and raw.strip():
                    all_results.append({"query": safe_q, "title": "", "url": "", "content": raw, "full_content": raw})
                    ...
                    for r in raw:
                        if isinstance(r, str) and r.strip():
                            all_results.append({"query": safe_q, "title": "", "url": "", "content": r, "full_content": r})
    ...
    def _build_supplement(self, search_results, logic_feedback, topic, history_block):
        results_block = ""
...
사례를 나열하는 데 그치지 말고, 그 사례가 토론에서 어떤 역할을 할 수 있는지 쉽고 자연스럽게 알려주세요.
