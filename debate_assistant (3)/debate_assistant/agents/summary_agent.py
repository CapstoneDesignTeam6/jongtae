...
- 단순 나열 금지. 주장들이 논리적 흐름으로 이어지도록 서술.
  예) "A를 주장했고, 이를 뒷받침하기 위해 B를 근거로 들었습니다. 나아가 ~"
- 상대 주장을 받아칠 때는 레이블·콜론·줄바꿈 없이 문단 흐름 안에서 자연스럽게 연결할 것.
"지적", "반론" 같은 단어를 활용해
  "상대의 [공격/지적/반론]에 대해서는 ~라고 말했습니다." 형식으로 서술.
  예) "...라고 강조했습니다. 상대의 디지털 피로 지적에 대해서는
      사무실에서도 동일하게 발생한다고 말했습니다. 나아가 ~"
- "재반박", "반박" 같은 메타 표현 절대 금지.
- 어느 쪽이든 상대 주장 없이 대응만 단독으로 서술하는 것 절대 금지.
- 각 측 문단은 200~350자. 너무 짧으면 안 됨.

아래 형식으로 출력. 이 두 섹션만 출력하고 다른 내용 절대 추가 금지:

[{self.user_label}]
(서술형 문단 — 주장의 흐름과 상대 공격·지적에 대한 대응이 자연스럽게 이어지도록)
